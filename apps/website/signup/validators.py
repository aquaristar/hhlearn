...
def custom_validator_password_requirement(self):
    ...
    if not re.match('^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{5,32}$', data):
        raise forms.ValidationError("Password must be at least 5 characters and must include at least one upper case letter, one lower case letter, and one numeric digit.")
    return data
